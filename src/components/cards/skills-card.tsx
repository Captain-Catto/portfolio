...
export function SkillsCard({ className }: SkillsCardProps) {
  ...
  return (
    ...
    >
      <div className="h-full flex flex-col pt-3 sm:pt-4 pb-2">
        {/* Header - Fixed */}
            ...
            <div className="flex flex-wrap justify-center items-start gap-2 pb-2">
              {skills.map((skill, index) => (
                <article
                  key={index}
                  className={cn(
                    "skill-card-shake rounded-lg shadow-md py-1 hover:bg-muted/30 transition-all duration-300 flex-shrink-0 hover:shadow-[0_0_20px_hsl(var(--primary)/0.6)] hover:border hover:border-primary/50 cursor-pointer",
                    skill.icon ? "bg-muted/20 pl-1 pr-3 sm:pr-4" : "bg-muted/20 px-2 sm:px-3"
                  )}
                >
                  <div className="flex justify-start items-center gap-1">
                    {skill.icon && (
                      <article className="bg-muted/30 rounded-lg p-1.5 sm:p-2 shadow-md">
                        <Image
                          src={skill.icon}
                          alt={skill.name}
                          width={24}
